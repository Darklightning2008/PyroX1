...
# Command handler to toggle command states
@bot.on_message(filters.command("enable") & filters.user(OWNER_ID))
async def toggle_command_enable(client, message):
    ...
    if command_state:
        ...
        return await message.reply_text(f"Command {command_name} enabled successfully!")

# Command handler to toggle command states
@bot.on_message(filters.command("disable") & filters.user(OWNER_ID))
async def toggle_command_disable(client, message):
    user_id = message.from_user.id
    command_name = message.text.split()[1].lower()
    ...
    if not command_state:
        ...
        return await message.reply_text(f"Command {command_name} disabled successfully!")