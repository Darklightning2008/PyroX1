...
@barath.on_message(filters.from_user(source_bot_id) & ~filters.forwarded)
async def forward_message(client, message):
    ...
    def from_target_bot(_, __, update):
        return update.from_user and update.from_user.id == int(target_bot_id)
    
    # Wait for a response from the target bot
    response = await barath.listen(from_target_bot)
